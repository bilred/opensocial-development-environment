...
public class MediaItemImplTest extends AbstractEntityTest {
	...
	private MediaItemImpl createMediaItem() {
		...
		mediaItem.setThumbnailUrl("thumbnailUrl1");
		return mediaItem;
	}
	...
	public void testSimple() throws Exception {
		...
		assertEquals("thumbnailUrl", "thumbnailUrl1", mediaItem.getThumbnailUrl());
		//
		tx = session.beginTransaction();
		mediaItem = (MediaItemImpl)session.get(MediaItemImpl.class, oid);
		...
		mediaItem.setThumbnailUrl("thumbnailUrl2");
		session.update(mediaItem);
		tx.commit();
		session.clear();
		tx = session.beginTransaction();
		...
		assertEquals("thumbnailUrl", "thumbnailUrl2", mediaItem.getThumbnailUrl());
		//
		tx = session.beginTransaction();
		mediaItem = (MediaItemImpl)session.get(MediaItemImpl.class, oid);
