...
//	}
	...
	public void testSimple() throws Exception {
		...
		name.setFormatted("unstructured1");
		session.save(name);
		long oid = name.getObjectId();
		tx.commit();
		session.clear();
		...
		assertEquals("unstructured", "unstructured1", name.getFormatted());
		tx.commit();
		//
		tx = session.beginTransaction();
		...
		name.setFormatted("unstructured2");
		session.update(name);
		tx.commit();
		session.clear();
		tx = session.beginTransaction();
		...
		assertEquals("unstructured", "unstructured2", name.getFormatted());
		tx.commit();
		//
		tx = session.beginTransaction();
		name = (NameImpl)session.get(NameImpl.class, oid);
