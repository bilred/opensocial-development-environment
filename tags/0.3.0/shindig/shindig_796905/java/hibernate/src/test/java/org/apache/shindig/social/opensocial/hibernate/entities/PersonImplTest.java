...
//		lookingFor2.setPerson(person);
	...
	public void testSimple() throws Exception {
		...
		person.setPreferredUsername("preferredUsername1");
		session.save(person);
		long oid = person.getObjectId();
		tx.commit();
		session.clear();
		...
		assertEquals("preferredUsername", "preferredUsername1", person.getPreferredUsername());
		tx.commit();
		//
		tx = session.beginTransaction();
		person = (PersonImpl)session.get(PersonImpl.class, oid);
		...
		person.setPreferredUsername("preferredUsername2");
		session.update(person);
		tx.commit();
		session.clear();
		//
		...
		assertEquals("preferredUsername", "preferredUsername2", person.getPreferredUsername());
		tx.commit();
		//
		tx = session.beginTransaction();
		person = (PersonImpl)session.get(PersonImpl.class, oid);
	...
	public void testAppData() throws Exception {
		...
		target.setAppData(expected);
		Map<String, ? extends Object> actual = target.getAppData();
		assertSame("appData", expected, actual);
	}
