...
/**
 * The Action for gadget files to iGoogle.
 *
 * @author albert.cheng.ig@gmail.com
 */
public class IgHostFileAction
    ...
    public void run(IAction action) {
        logger.fine("in run");
        IgHostFileDialog dialog = new IgHostFileDialog(shell);
        int openResult = dialog.open();
        if (openResult == Window.OK) {
            String username = dialog.getUsername();
            String password = dialog.getPassword();
            String hostProjectName = dialog.getHostProjectName();
            Job job = new IgHostFileJob(username, password, hostProjectName, gadgetXmlIFile, shell);
            job.schedule();
        }
    }
