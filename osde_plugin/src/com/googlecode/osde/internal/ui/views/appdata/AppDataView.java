        ...
        if (Activator.getDefault().isRunningShindig()) {
            try {
                ...
                ApplicationImpl application =
                        (ApplicationImpl) applicationCombo.getData(applicationCombo.getText());
                AppDataService appDataService = Activator.getDefault().getAppDataService();
                Map<String, String> dataMap =
                        appDataService.getApplicationDataMap(person, application);
                keyList.removeAll();
                valueText.setText("");
                if (dataMap != null) {
        ...
        public void widgetSelected(SelectionEvent evt) {
            if (Activator.getDefault().isRunningShindig()) {
                try {
                    ...
                    if (MessageDialog.openConfirm(getSite().getShell(), "Confirm",
                            "Would you like to delete the [" + key + "] value?")) {
                        AppDataService appDataService = Activator.getDefault().getAppDataService();
                    ...
                    if (dialog.open() == Window.OK) {
                        try {
                            ...
                            AppDataService appDataService =
                                    Activator.getDefault().getAppDataService();
